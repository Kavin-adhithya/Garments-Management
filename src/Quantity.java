...
public class Quantity extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			String s4 = request.getParameter("value1");
			HttpSession session=request.getSession();
			session.setAttribute("usquantity",s1);
			session.setAttribute("ussize",s2);
			session.setAttribute("uscolor",s3);
			session.setAttribute("uscat",s4);
